...
{
  printf( "Usage:\n"
    "    buffer_argument_testcase [-d domain] [-U unsigned_PD] [-r run_locally] \n\n"
    "Options:\n"
...
int main(int argc, char* argv[])
{
  ...
  printf("\nStarting buffer_argument_testcase test\n");
  printf("Attempting to run on %s PD on domain %d\n",
               is_signedpd_requested ? "signed" : "unsigned", domain_id);
  nErr = buffer_argument_testcase_test( domain_id, is_signedpd_requested);
  if (nErr) {
    printf("ERROR 0x%x: buffer_argument_testcase test failed\n\n", nErr);
    goto bail;
  }
