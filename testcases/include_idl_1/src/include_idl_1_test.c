...
int local_include_idl_1_sum(int* vec, int vecLen)
{
    int sum=0;
...
int include_idl_1_test(int domain, int num, bool is_signedpd_requested) {
  ...
  printf("- creating sequence of numbers from 0 to %d\n", num - 1);
    ...
    if (domain == ADSP_DOMAIN_ID)
      uri = include_idl_1_URI ADSP_DOMAIN;
    else if (domain == CDSP_DOMAIN_ID)
      uri = include_idl_1_URI CDSP_DOMAIN;
    else if (domain == MDSP_DOMAIN_ID)
      uri = include_idl_1_URI MDSP_DOMAIN;
    else if (domain == SDSP_DOMAIN_ID)
      uri = include_idl_1_URI SDSP_DOMAIN;
    else {
