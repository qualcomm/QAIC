...
{
  printf( "Usage:\n"
    "    include_idl_1 [-d domain] [-U unsigned_PD] -n array_size\n\n"
    "Options:\n"
...
int main(int argc, char* argv[])
{
  ...
  printf("\n- Starting include_idl_1 test on domain %d\n", domain);
  nErr = include_idl_1_test(domain, num, is_signedpd_requested);

bail:
  if (nErr) {
    printf("- include_idl_1 example failed with nErr = %d\n\n", nErr);
  } else {
    printf("- success\n\n");
  }
