...
{
  printf( "Usage:\n"
    "    test_mt_31_methods [-d domain] [-U unsigned_PD] -n array_size\n\n"
    "Options:\n"
...
int main(int argc, char* argv[])
{
  ...
  printf("\n- Starting test_mt_31_methods test on domain %d\n", domain);
  nErr = test_mt_31_methods_test(domain, num, is_signedpd_requested);

bail:
  if (nErr) {
    printf("- test_mt_31_methods example failed with nErr = %d\n\n", nErr);
  } else {
    printf("- success\n\n");
  }
