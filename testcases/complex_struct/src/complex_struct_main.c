...
{
  printf( "Usage:\n"
    "    complex_struct [-d domain] [-U unsigned_PD] -n array_size\n\n"
    "Options:\n"
...
int main(int argc, char* argv[])
{
  ...
  printf("\n- Starting complex_struct test on domain %d\n", domain);
  nErr = complex_struct_test(domain, num, is_signedpd_requested);

bail:
  if (nErr) {
    printf("- complex_struct example failed with nErr = %d\n\n", nErr);
  } else {
    printf("- success\n\n");
  }
