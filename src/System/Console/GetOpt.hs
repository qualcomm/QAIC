...
-- Modifications:
--
-- * gregf@qualcomm.com: Added support for long options with a single dash
-----------------------------------------------------------------------------
...
import Data.List ( isPrefixOf, find, stripPrefix )
...
fmtOpt (Option sos los ad descr) =
   ...
   where sepBy _  []     = ""
         ...
         sosFmt = fmtShort ad sos
         losFmt = sepBy ',' (map (fmtLong  ad) los)

fmtShort :: ArgDescr a -> String -> String
fmtShort _             "" = ""
...
getNext ('-': xs) rest optDescr    = shortOpt xs rest optDescr
--getNext ('-': x :xs) rest optDescr | null [ o  | o@(Option ss _ _ _) <- optDescr, s <- ss, x == s ]
--                                   = longOpt (x:xs) rest optDescr
...
-- handle short option
shortOpt :: String -> [String] -> [OptDescr a] -> (OptKind a,[String])
shortOpt ls rs optDescr
      | null options                          = (UnreqOpt ("-"++ls),rs)
      | length options > 1                    = (errAmbig (map fst options) optStr, rs)
      | otherwise                             = short ad (if null arg then arg'' else arg) rs
   where (opt,arg) = break (=='=') ls
         getWith p = [ (o, fromJust (stripPrefix xs opt)) | o@(Option xs _ _ _) <- optDescr, not (null xs) && p xs opt]
         exact     = getWith (==)
         options   = if null exact then getWith isPrefixOf else exact
         (Option _ _ ad _, arg')              = head options
         arg''                                = if null arg' then "" else '=':arg'
         optStr    = ("-"++opt)

         short (NoArg  a  ) []       rest     = (Opt a,rest)
         short (NoArg  _  ) ('=':_)  rest     = (errNoArg optStr,rest)
         short (ReqArg _ d) []       []       = (errReq d optStr,[])
         short (ReqArg f _) []       (r:rest) = (Opt (f r),rest)
         short (ReqArg f _) ('=':xs) rest     = (Opt (f xs),rest)
         short (OptArg f _) []       rest     = (Opt (f Nothing),rest)
         short (OptArg f _) ('=':xs) rest     = (Opt (f (Just xs)),rest)
         short _            _        rest     = (UnreqOpt ("-"++ls),rest)
