...
import Language.Idl.ParserState(ParserState)
...
data TokenParser st
    = TokenParser{ identifier       :: CharParser st String
                 ...
                 , oneLineComment   :: CharParser ParserState ()
                 , multiLineComment :: CharParser ParserState ()
                 }

-----------------------------------------------------------
-- Given a LanguageDef, create a token parser.
-----------------------------------------------------------
makeTokenParser :: LanguageDef ParserState -> TokenParser ParserState
makeTokenParser languageDef
    ...
    --whiteSpace
    whiteSpace
        | noLine && noMulti  = skipMany (simpleSpace <|> preprocessor <?> "")
        | noLine             = skipMany (simpleSpace <|> preprocessor <|> multiLineComment <?> "")
        | noMulti            = skipMany (simpleSpace <|> preprocessor <|> oneLineComment <?> "")
        | otherwise          = skipMany (simpleSpace <|> preprocessor <|> oneLineComment <|> multiLineComment <?> "")
        where
          noLine  = null (commentLine languageDef)
          noMulti = null (commentStart languageDef)

    preprocessor = do { char '#'; pragma <|> linePreprocessor }

    pragma = do { try (string "pragma"); skipMany (satisfy (/= '\n')) }

    linePreprocessor =
        do { optionMaybe (string "line")
           ; simpleSpace
           ; line <- nat
           ; simpleSpace
           ; filepath <- filepathLiteral
           ; simpleSpace
           ; _flg <- optionMaybe nat
           ; setPosition (newPos filepath (fromInteger line) 1)
           }

    filepathLiteral = between doubleQuote doubleQuote filepath
       where
          doubleQuote = char '"'
          filepath    = fmap unescape (many (satisfy (/= '"')))

          -- RVCT Preprocessor separates directories
          -- by two backslashes.  Unescape.
          unescape          = concatMap undouble . group
          undouble ('\\':_) = ['\\']
          undouble x        = x
    ...
    oneLineComment =
        do{ try (string (commentLine languageDef)) <?> ""
          ; pos  <- getPosition
          ; cnts <- many (satisfy (/= '\n'))
          ; updateState ((pos, Right cnts) :)
          }

    multiLineComment =
        do { try (string (commentStart languageDef))
           ; pos  <- getPosition
           ; cnts <- inCommentSingle
           ; updateState ((pos, Left cnts) :)
           }

    inCommentSingle
        =   do{ try (string (commentEnd languageDef)); return "" }
        <|> do{ s1 <- many1 (noneOf startEnd)        ; s2 <- inCommentSingle; return (s1++s2) }
        <|> do{ ch <- oneOf startEnd                 ; s <- inCommentSingle; return (ch:s) }
        <?> "end of comment"
        where
